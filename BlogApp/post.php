...
// Fetch the blog post using prepared statement
...
<main>
    <section class="single-blog">
        ...
        <?php if(!empty($row['image'])): ?>
            <img src="assets/images/<?php echo $row['image']; ?>" alt="<?php echo htmlspecialchars($row['title']); ?>" class="blog-image">
        <?php endif; ?>
        ...
        <div class="comments">
            ...
            if(isset($_POST['submit_comment'])){
                if(!isset($_SESSION['user_id'])){
                    ...
                    if(!empty($comment_text)){
                        ...
                        $stmt->bind_param("iis", $id, $_SESSION['user_id'], $comment_text);
                        $stmt->execute();
                        $stmt->close();
            ...
            // Fetch comments
            $comments_result = $conn->query("SELECT comments.*, users.username FROM comments JOIN users ON comments.user_id = users.id WHERE post_id = $id ORDER BY created_at DESC");
